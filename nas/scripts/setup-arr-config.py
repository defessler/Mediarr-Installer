...
def render_homepage_custom_css():
    """Monokai for the dashboard, matching the two sidecar pages and the
    docs site so the whole stack reads as one hand.

    Homepage is a Tailwind app themed through a --color-50..950 ramp, so
    redefining that ramp in Monokai greys re-skins the chrome without
    chasing individual selectors. If a future Homepage renames those
    variables the ramp is simply ignored and the font and accent rules
    still apply, which is why this leans on them rather than on deep
    element selectors."""
    return """/* Generated by setup-arr-config.py — Mediarr Monokai theme.
   Safe to delete: Homepage works fine without it, and the wizard will
   write it again on the next run. To keep your own version, remove the
   marker line above and it will be left alone. */

:root {
  /* Monokai greys, light to dark. Homepage's dark theme draws its
     surfaces from the high end and its text from the low end. */
  --color-50:  248, 248, 242;
  --color-100: 230, 227, 216;
  --color-200: 200, 196, 180;
  --color-300: 168, 164, 147;
  --color-400: 138, 134, 118;
...
/* Service tiles: a flat surface with a hairline border rather than a
   drop shadow, and a green edge on hover so the pointer target is
   obvious without the card jumping. */
.service-card, .bookmark-card {
  border: 1px solid rgb(73, 72, 62);
  border-radius: 3px;
...
/* Group headings in the same code-comment idiom as everything else. */
.services-group > h2, #information-widgets h2 {
  color: rgb(166, 226, 46);
  font-weight: 700;
  letter-spacing: 0.03em;
}

/* Status dots read as traffic lights, so keep them literal. */
.status-online  { color: rgb(166, 226, 46) !important; }
.status-offline { color: rgb(249, 38, 114) !important; }

a:hover { color: rgb(102, 217, 239); }

/* Scanlines, matching the sidecars. Pointer-events none so it never
   intercepts a click on a tile. */
body::after {
  content: '';
  position: fixed;
  inset: 0;
...
def render_homepage_settings(env):
    ...
    out = ["# Homepage settings — generated by setup-arr-config.py",
           ...
           "startUrl: /",
           "theme: dark",
           # Monokai's background is a warm grey. Slate is blue-tinted and
           # fought the green/pink accents custom.css layers on top, so
           # neutral is the closer starting point.
           "color: neutral",
           "",
           "layout:"]
    # Media — gated on Plex
...
def main():
    ...
    if not is_enabled(env, 'ENABLE_HOMEPAGE'):
        ...
        # custom.css — the Monokai skin. Backed up first like the rest, and
        # only overwritten when it still carries our generated marker, so
        # a hand-rolled theme survives a re-run.
        css_path = f"{homepage_cfg}/custom.css"
        existing_css = ''
        if os.path.exists(css_path):
            try:
                ...
                pass
        if existing_css and 'Generated by setup-arr-config.py' not in existing_css:
            skip("Homepage custom.css (hand-edited — left alone)")
        else:
            backup_before_overwrite(css_path)
            overwrite_config_file("Homepage theme",
                css_path, render_homepage_custom_css())
