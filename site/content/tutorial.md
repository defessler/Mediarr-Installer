...
description: "Build the whole stack by hand to understand every layer: Docker, Compose, networks, hardlinks, the VPN, and the arr APIs."
lede: "Build the whole stack by hand to understand every layer: Docker, Compose, networks, hardlinks, the VPN, and the arr APIs."
group: "Getting started"
order: 2
nav: "Tutorial"
navOrder: 3
---
This tutorial teaches you how to build the Mediarr stack from scratch, even if you've **never used Docker, SSH, the command line, or any of the underlying tools**. By the end, you'll understand every layer, and you'll be able to extend, customize, or rebuild any of it from memory.

It's a teaching document. If you just want a working stack, download the installer from [Releases](https://github.com/defessler/Mediarr-Installer/releases/latest). That's the eight-screen wizard described in [INSTALL.md](Installation). The wizard does everything below for you in 20 minutes.
...
- A computer (Windows, Mac, or Linux). For Chapters 1 to 7, your own laptop works. Later chapters assume access to a Linux box, ideally a Synology NAS, but any Linux machine with Docker installed works.
- A web browser.
...
- About four hours total. Each chapter is 20 to 30 minutes.
...
| `-d` | "Detached", run in the background and don't tie up the terminal. |
...
Same nginx, same port 8080, but now declared in a file. Visit http://localhost:8080 and it still works. `docker compose down` to stop everything.
...
- **Keys end with `:` and lists start with `- `.** That's why `ports:` is followed by `- "8080:80"`.
...
This is how the Mediarr stack avoids hardcoded IPs everywhere. Sonarr connects to qBittorrent at `http://gluetun:49156`. Bazarr connects to Sonarr at `http://sonarr:8989`. None of those would survive a NAS-IP change if they were IPs. Service names are stable.
...
Two paths. One file. No copy. Delete one path and the other still works. qBittorrent keeps seeding from its path, and Plex reads from the tidier path. **You save the size of every media file you have.**
...
- http://localhost:49156 - qBittorrent (default login `admin` / `adminadmin`, change it in Settings)
...
Re-run it. It'll print "already configured", which is the **idempotent** property. The real `setup-arr-config.py` in the Mediarr repo is 3500 lines because it covers every service + every config knob, but every section follows this same `GET → compare → POST if needed` pattern.
...
1. **A way to upload all of the above to a remote NAS over SSH.**
   The installer uses `ssh2` (a Node library) to open an SSH session + `sftp` (a subsystem of SSH) to bulk-upload files. The renderer kicks off uploads, and the main process streams progress back via IPC.
...
The companion [TDD.md](https://github.com/defessler/Mediarr-Installer/blob/master/TDD.md) explains *why* each of those pieces is the shape it is.
...
- [linuxserver.io docs](https://docs.linuxserver.io) - the docker images we use, and their PUID/PGID conventions matter.

### Mediarr-specific deep dives

- **[README.md](https://github.com/defessler/Mediarr-Installer/blob/master/README.md)** - architecture overview + manual install path.
- **[INSTALL.md](Installation)** - installer wizard walkthrough.
- **[TDD.md](https://github.com/defessler/Mediarr-Installer/blob/master/TDD.md)** - every design decision explained.
