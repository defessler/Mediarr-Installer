...
**Sonarr** manages TV shows and **Radarr** manages movies. They work identically:
...
4. Once the download finishes, they **import and rename** the file into your media library using a hardlink, so no extra disk space is used.
...
3. The **root folder** (`/data/Media/Movies`) and **quality profile** are already set by the installer, so leave them as they are unless you have a reason to change.
4. Click **Add Movie**. Radarr searches immediately, and if a release exists it starts downloading.
...
- **Nothing is downloading?** The most common cause is that no release matched, either too few indexers (add more in Prowlarr) or a quality profile that's too strict (loosen it in [Quality Profiles](Quality-Profiles)). Check **Wanted → Missing** and look at the search results icon next to the title for clues.
