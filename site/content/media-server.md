...
order: 3
---
This guide covers the app that actually **streams your library** to TVs, phones, and browsers — Plex or Jellyfin, whichever you chose when you ran the installer.
...
- **Plex** (default) — streams to every platform via the Plex app; requires a free Plex account. Open it at **`http://<NAS-IP>:32400/web`**.
...
1. Open **`http://<NAS-IP>:32400/web`** directly in your browser — go to the IP address, not the cloud URL. If the server shows up with a hashed/random name and won't let you in, it hasn't been claimed yet; opening it via the direct IP link fixes that.
2. Sign in with your Plex account. Plex will claim this server to your account automatically.
3. Your libraries are already created and pointed at the right folders — click through the setup wizard and accept the defaults.
...
4. Your libraries are already set up — click through the wizard and accept the defaults.
...
- **Want to add content?** Use [Requesting Movies & TV](Requesting-Movies-and-TV) — it's the easiest path. For direct queue management, use [Movies & TV](Movies-and-TV).
