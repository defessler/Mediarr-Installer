---
title: "Installation"
description: "Step-by-step walkthrough of the wizard, from enabling SSH to your first download."
lede: "Step-by-step walkthrough of the wizard, from enabling SSH to your first download."
...
organizes, and streams movies/TV/music to any device on your home network, all
hosted on your NAS.

> If you are comfortable with SSH and bash, the manual install path is in
> [`README.md`](https://github.com/defessler/Mediarr-Installer/blob/master/README.md). This guide uses the GUI installer (`Mediarr
...
never touch them directly, since they talk to each other behind the scenes. Here's
...
| **Gluetun** | The VPN wrapper | Acts as a sealed envelope around qBittorrent. All torrent traffic exits through your VPN provider. If the VPN drops, qBit can't reach anything, so no IP leaks. You only see this container if you enabled VPN in the wizard. |
| **SABnzbd** | The usenet downloader | Pulls files from Usenet, which is faster and more reliable than torrents, but requires a paid provider (Eweka, Newshosting, etc.). Optional. |
...
| **recyclarr-trigger** | The Recyclarr web UI | Tiny webhook server at `http://<NAS>:8889`. Recyclarr is a CLI tool with no UI of its own, so this page surfaces **profile dropdowns** (pick a different TRaSH bundle without re-running the installer) plus a **Sync Now** button. Reachable from the Homepage dashboard's Maintenance tile. |
...
**The shortest version of the whole stack:** *Seerr* takes requests → *Sonarr / Radarr / Lidarr* watch for what you want → *Prowlarr* tells them where to look → *qBittorrent + Gluetun* and *SABnzbd* download it → *Unpackerr* unpacks it if needed → *Bazarr* fetches subtitles → *Plex* plays it. *Homepage* is the dashboard linking to everything, *Tautulli* shows watch stats, and *Recyclarr* keeps quality settings sane.
...
| **Your NAS's IP address** on your home network | The installer needs to find it | DSM → Control Panel → Network, where the LAN IP is shown |
| **Your NAS admin password** | To log in over SSH | Same one you use to sign into DSM |
| **About 30 minutes of patience** | First-time install downloads a lot of Docker images | n/a |
...
| **An indexer or two** | Where the *arr apps find releases | Prowlarr ships with public indexers built-in. For private ones, see [Step 4](#6-step-4-first-time-setup-for-each-service) |
...
3. Find **Terminal & SNMP** (it's near the bottom of the list, so you may need
...
XCircle (red) icon so you can tell pass / fail in shape, not just color.
This is useful if you're color-blind or have a dim screen.
...
1. **Services + media server** - uncheck anything you don't want. The
   **Media server** row has a **Plex / Jellyfin** picker, so choose **Jellyfin**
...
3. **Plex claim token** *(Plex only, skip if you picked Jellyfin)* - this
   ...
   **Jellyfin** has no claim token. Install right away, then open
...
6. **Indexers** *(optional, skip on first install and add them later)*:
...
7. **Identity** - usually fine as it is. This is which Linux user inside the
...
15 to 25 minutes because Docker has to download about 3 GB of images.
...
shimmer animation on its leading edge, so you can see the install is alive
even when the log is quiet. Steps tick over from gray → amber-spinning →
...
a new claim token, and keep it handy, because Plex will need it. The wizard
...
- **Sonarr / Radarr step:** the wizard wires these up automatically, so pick
...
**End-to-end: typically 5 to 30 minutes for popular content.** For something
...
| Seerr shows "Port 5056 connection refused" | First-run wizard not yet completed | Open Seerr in a browser and finish its setup wizard |
...
| qBittorrent shows "Firewalled" / port forwarding doesn't work | Using NordVPN, where gluetun doesn't support PF | Either accept reduced seed ratio, or switch `VPN_PROVIDER` to `protonvpn` / `pia` / `privatevpn` |
...
| Plex doesn't show new files within a minute of import | Plex Connect notification missing | The wizard configures this automatically. If it's missing, add it manually in Sonarr/Radarr → Settings → Connect → Plex Media Server (host=plex, port=32400) |
...
Safe to do any time, because the wizard's idempotent. It detects what's already
...
- **Change quality profiles whenever you want** - open the Recyclarr tile
  ...
  and runs the sync in one click, with no need to edit files or re-run the
  installer.
