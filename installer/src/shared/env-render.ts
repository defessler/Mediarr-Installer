...
export interface EnvFormValues {
   ...
   *  profile. ENABLE_PLEX stays the on/off master for the media-server
   *  group (server + request manager) regardless of which one runs. */
  MEDIA_SERVER?: string
  /** Request-manager container image. Derived from MEDIA_SERVER by
   *  renderEnv (jellyseerr for Jellyfin — a superset fork that supports
   *  Plex AND Jellyfin; seerr for Plex). Emitted so the .env is self-
   *  contained for `docker compose`. */
  SEERR_IMAGE?: string
  ...
  // ── Jellyfin
  /** Jellyfin API key (Dashboard → API Keys), pasted after the first-run
   *  setup at http://<NAS>:8096 so setup-arr-config.py can wire arr →
   *  Jellyfin library scans and the request manager. Blank until the
   *  user generates one. Ignored when MEDIA_SERVER=plex. */
  ...
  // ── Storage analysis (LibrARRian) — OPT-IN like Soulseek (isOptInEnabled).
  /** Whether to install LibrARRian: a sidecar that reports which media is
   *  eating the array, at what quality, and (with Tautulli/Jellyfin) what has
   *  never been played. Needs no credentials of its own — it reads the arr API
   *  keys already in .env. A missing key counts as OFF. */
  ENABLE_LIBRARIAN?: string
  /** Whether LibrARRian may CHANGE things (set a quality profile, trigger a
   *  search, delete a file through the arr) as well as report on them, i.e.
   *  write mode. ON by default, and evaluated with isEnabled (default-on),
   *  NOT isOptInEnabled — only an explicit false/0/no/off turns it off. Keep
   *  that in step with actions_enabled() in librarian.py. It stays a separate
   *  key from ENABLE_LIBRARIAN so a LAN you don't fully trust can run the
   *  report read-only without giving up the report itself. */
...
export function renderEnv(v: EnvFormValues): string {
  ...
  // request-manager image the stack pulls.
  const mediaServer = (v.MEDIA_SERVER || 'plex').trim().toLowerCase() === 'jellyfin'
    ? 'jellyfin' : 'plex'
  return [
    ...
    '',
    '# Media server — plex (default) or jellyfin. ENABLE_PLEX (above) is',
    '# the on/off master for the media-server group; MEDIA_SERVER picks',
    '# which one runs. setup.sh activates the matching compose profile.',
    line('MEDIA_SERVER', mediaServer),
    // Request-manager image. Jellyseerr (superset fork — Plex + Jellyfin)
    // for Jellyfin; Seerr for Plex so existing installs stay byte-
    // identical. docker-compose.yml reads ${SEERR_IMAGE:-<seerr default>}.
    line('SEERR_IMAGE', mediaServer === 'jellyfin'
      ? 'ghcr.io/fallenbagel/jellyseerr:latest'
      : 'ghcr.io/seerr-team/seerr:latest'),
    '',
    '# Plex — claim token from https://plex.tv/claim (expires in 4 min,',
    '# only needed on first run). Ignored when MEDIA_SERVER=jellyfin.',
    line('PLEX_CLAIM', mediaServer === 'jellyfin' ? '' : v.PLEX_CLAIM),
    '',
    '# Jellyfin — API key (Dashboard → API Keys), pasted after the first-',
    '# run setup at http://<NAS>:8096. Lets the installer wire arr library',
    '# scans + the request manager. Ignored when MEDIA_SERVER=plex.',
    line('JELLYFIN_API_KEY', mediaServer === 'jellyfin' ? v.JELLYFIN_API_KEY : ''),
    '',
    '# ARR Web UI Auth',
    ...
    '',
    '# Storage analysis (LibrARRian). OPT-IN; off by default. A missing',
    '# ENABLE_LIBRARIAN key counts as OFF (like ENABLE_SOULSEEK) — emitted via',
    '# isOptInEnabled. Report on ${LAN_IP}:8890 showing what is eating the',
    '# array, at what quality, and what has never been played.',
    line('ENABLE_LIBRARIAN', isOptInEnabled(v.ENABLE_LIBRARIAN) ? 'true' : 'false'),
    '# Re-grab actions (write mode). ON by default. Note this uses isEnabled,',
    '# NOT isOptInEnabled: only an explicit false/0/no/off turns it off, which',
    '# is exactly what actions_enabled() in librarian.py does. Set it false for',
    '# a read-only page. Deletes additionally refuse unless the arr has a',
