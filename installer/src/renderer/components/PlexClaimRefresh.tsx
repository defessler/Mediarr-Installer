...
// plex.tv/claim. Lives on the Run screen right above the Start button.
...
export function PlexClaimRefresh({ value, onChange }: Props) {
  // Track when the value last changed so we can compute remaining time.
  const enteredAtRef = useRef<number>(value ? Date.now() : 0)
  const lastValueRef = useRef<string | undefined>(value)
  const [, setTick] = useState(0)

  // Reset the countdown whenever the value changes (user pastes new one).
  useEffect(() => {
    if (value !== lastValueRef.current) {
      lastValueRef.current = value
      enteredAtRef.current = value ? Date.now() : 0
    }
  }, [value])

  // Repaint every second so the countdown ticks down visibly.
  useEffect(() => {
    if (!value) return
  ...
  const ageMs = value && enteredAtRef.current ? Date.now() - enteredAtRef.current : 0
