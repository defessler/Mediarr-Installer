...
const ITEMS: TItem[] = [
  ...
  // who scans the entry list still finds the tutorial.
  {
    ...
    cause:
      'You don\'t need to — the wizard handles all of that. There\'s a full beginner\'s walkthrough on the documentation site that covers everything from enabling SSH on your NAS through adding your first show in Sonarr/Radarr.',
    fix:
      'Open https://dougfessler.com/Mediarr-Installer/installation/ in your browser. It walks through the whole install with ASCII mockups of each wizard screen, what to type in each field, and what to do once each service is running. The same site has an architecture page with diagrams if you want to understand what the wizard actually built.',
  },
...
sudo bash setup.sh`,
  ...
  },
  {
    ...
    fix:
      'Run the bundled fix-imports.sh helper. It (1) fires DownloadedEpisodesScan / DownloadedMoviesScan / DownloadedAlbumsScan against all six known completed-download paths (torrent + usenet roots for each arr), (2) waits 30s for the arrs to process, (3) dumps any items still stuck along with their exact statusMessages, (4) reports library + backlog file counts so you can see whether imports actually landed. This same script auto-runs as Step 12 of every install now, so the symptom shouldn\'t recur on fresh installs.',
...
# https://trash-guides.info/Lidarr/lidarr-setup-quality-profiles/`,
  },
  {
    ...
    fix:
      'Enable LibrARRian on the Configure screen. It reads size and quality straight out of Sonarr, Radarr, and Lidarr and serves one page on port 8890 (there\'s a tile for it under Maintenance on Homepage). It ranks your biggest items by bytes per hour, which is the number that finds bloated remuxes. Raw size only ever finds long shows. It also shows space grouped by quality tier, each arr\'s cutoff-unmet count, and, when Tautulli or Jellyfin can tell it, what is large and has never been played. Every column sorts, and you can filter by size or by bytes per hour. The same report runs from SSH if you\'d rather not enable the container. Write mode is on by default, so upgrading and shrinking are there too. Unticking "Allow re-grab actions" on the Configure screen makes it read-only: it then issues nothing but GETs and cannot edit a profile, trigger a search, or delete a file. That stays a separate switch from the report because the page has no login.',
    command: (c) =>
      `# Browser (needs ENABLE_LIBRARIAN=true and a wizard re-run):\n`
      + `#   http://<NAS>:8890  (or the LibrARRian tile on Homepage)\n\n`
        ...
        <header className="px-5 pt-4 pb-3 border-b border-slate-800">
          ...
          <div className="mt-2 rounded-md border border-emerald-700/40 bg-emerald-900/15 px-3 py-2 text-xs">
            ...
            <a
              ...
              href="https://dougfessler.com/Mediarr-Installer/installation/"
              target="_blank"
              rel="noreferrer"
              aria-label="Open the beginner's Installation walkthrough on the documentation site in a new tab"
            >
              Installation guide
